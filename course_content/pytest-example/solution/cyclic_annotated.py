...
import numpy as np  #> We'll need numpy.ma instead of ma to fix the masked
                    #> array bug
...
def add_cyclic_point(data, axis=-1):
    ...
    * cyclic_data:
    ...
    slicer[axis] = slice(0, 1)  #> This could fail with a seemingly obscure
                                #> error if the input is invalid
    #> The next line will unmask a masked array
